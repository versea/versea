...
export class Router implements IRouter {
  ...
  public match(): MatchedResult {
    ...
    if (this._config.routerMode === 'hash') {
      const wholePath = window.location.hash.substring(1);
      const queryIndex = wholePath.indexOf('?');
      path = wholePath.substring(0, queryIndex);
      query = parse(wholePath.substring(queryIndex + 1));
    } else {
